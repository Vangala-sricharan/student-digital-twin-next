      ...
      {projects.length > 0 ? (
              ...
              <div className="space-y-3">
                  ...
                  <div className="flex items-center gap-1.5">
                    {proj.githubUrl && (
                      <a
                        ...
                        target="_blank"
                        rel="noreferrer"
                        className="p-2 rounded-lg bg-slate-100 dark:bg-white/5 border border-slate-200 dark:border-white/10 text-slate-700 dark:text-slate-300 hover:text-blue-600 dark:hover:text-white transition-colors"
                    ...
                    {proj.liveUrl && (
                      <a
                        ...
                        target="_blank"
                        rel="noreferrer"
                        className="p-2 rounded-lg bg-slate-100 dark:bg-white/5 border border-slate-200 dark:border-white/10 text-slate-700 dark:text-slate-300 hover:text-blue-600 dark:hover:text-cyan-300 transition-colors"
                ...
                <div className="flex flex-wrap gap-1.5 pt-1">
                  {proj.techStack.map((t, idx) => (
                    <span
                      key={idx}
                      className="px-2.5 py-1 rounded-md bg-slate-100 dark:bg-white/5 border border-slate-200 dark:border-white/10 text-[11px] font-mono font-medium text-slate-700 dark:text-slate-300"
                    >
                      {t}
                    </span>
                  ))}
              ...
              <div className="pt-4 border-t border-slate-200 dark:border-white/5 grid grid-cols-2 gap-2 text-xs font-mono">
                ...
                <div className="p-2.5 rounded-xl bg-slate-50 dark:bg-white/5 border border-slate-200 dark:border-white/10">
                  ...
                  <div className="font-bold text-emerald-600 dark:text-emerald-400 mt-0.5">
                    {proj.entropyScore ? `${proj.entropyScore}% Organic` : 'Pending Audit'}
                  </div>
                </div>
              </div>
        ...
        <div className="p-12 text-center rounded-[2.5rem] bg-white dark:bg-[#0d1117] border border-slate-200 dark:border-white/10 space-y-4">
          ...
          <div className="space-y-1">
            <h3 className="text-base font-bold text-slate-900 dark:text-white">No Projects Indexed Yet</h3>
            <p className="text-xs text-slate-500 dark:text-slate-400 max-w-md mx-auto">
              Your Student Digital Twin starts with zero proof repositories. Add your codebases in My Profile or execute the Project Auditor engine to verify your AST evidence.
            </p>
          </div>
        </div>
      )}
