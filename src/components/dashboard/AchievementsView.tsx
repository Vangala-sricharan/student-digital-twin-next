...
import { Award, Trophy, Calendar, CheckCircle2 } from 'lucide-react';
    ...
    <div className="space-y-6 animate-in fade-in duration-200">
      ...
      {achievements.length > 0 ? (
            ...
            >
              <div className="space-y-3">
                <div className="flex items-start justify-between gap-3">
                  ...
                  <span className="text-[10px] font-mono px-2.5 py-1 rounded-md bg-slate-100 dark:bg-white/5 border border-slate-200 dark:border-white/10 text-slate-700 dark:text-slate-300 font-semibold uppercase">
                    {a.category}
                  </span>
                </div>
        ...
        <div className="p-12 text-center rounded-[2.5rem] bg-white dark:bg-[#0d1117] border border-slate-200 dark:border-white/10 space-y-4">
          <div className="w-12 h-12 rounded-2xl bg-amber-50 dark:bg-white/5 border border-amber-200 dark:border-white/10 text-amber-600 dark:text-amber-400 flex items-center justify-center mx-auto">
            <Award className="w-6 h-6" />
          </div>
          <div className="space-y-1">
            <h3 className="text-base font-bold text-slate-900 dark:text-white">No Milestones Recorded Yet</h3>
            <p className="text-xs text-slate-500 dark:text-slate-400 max-w-md mx-auto">
              Your Student Digital Twin starts with zero logged distinctions. Add academic awards, competitive milestones, and certifications in My Profile.
            </p>
          </div>
        </div>
      )}
